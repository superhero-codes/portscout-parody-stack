...
| 15  | **BeerFinder**    | Beer apps   | `projects/beerfinder`    | Node/Express                | 5325             | `npm run dev`                                         | ⚠️ Port conflict with Faceplant (intermittent — Faceplant crashes 50%)                                                                                                 | `package.json`, `server.js` with port 5325                               | HTTP 200 on `:5325` (if Faceplant isn't running)                                                         |
...
| 24  | **MySpice**       | MySpace     | `projects/myspice`       | PHP                         | 5323             | `php -S localhost:5323`                               | ⚠️ Port conflict with SubSnack (PHP vs Node), 2005-era MySpace UI, marquee tags, visitor counter, no modern tooling                                                    | `index.php`, no `package.json`, no `node_modules`                        | HTTP 200 on `:5323` (requires PHP, conflicts with SubSnack)                                              |
...
| 26  | **Napper**        | Napster     | `projects/napper`        | Ruby (Rack)                 | 5331             | `rackup -p 5331`                                      | ⚠️ Port conflict with CPUStorm (Ruby vs Node + CPU spikes), fake file-sharing UI with suspicious `.exe` files                                                          | `Gemfile`, `config.ru`, `app/controllers/`                               | HTTP 200 on `:5331` (requires Ruby + Rack, conflicts with CPUStorm)                                      |
...
| **Lotion + SpotiPie** | ⚠️ Port conflict (5301) | Both projects are configured to use port `5301`. Starting both simultaneously will cause one to fail with `EADDRINUSE`. This is intentional. |
| **BeerFinder + Faceplant** | ⚠️ Port conflict (5325) | Both use port `5325`. Intermittent conflict — Faceplant crashes 50% of the time, so BeerFinder sometimes gets the port. Node vs Node. |
| **MySpice + SubSnack** | ⚠️ Port conflict (5323) | Cross-technology conflict: PHP (`php -S localhost:5323`) vs Node.js (`npm run dev`). Both want port `5323`. |
| **Napper + CPUStorm** | ⚠️ Port conflict (5331) | Cross-technology conflict: Ruby/Rack (`rackup -p 5331`) vs Node.js (`npm start`). Both want port `5331`. CPUStorm also spikes CPU every 5 seconds. |
...
| **MySpice** | 📜 Requires PHP | Unlike every other server-based project, MySpice requires PHP (`php -S localhost:5323`). There is no `package.json`. Node-based tooling will not detect it as a project. Conflicts with SubSnack on port 5323. |
...
- [x] Detect that two projects share port 5301 (Lotion + SpotiPie)
- [x] Detect that two projects share port 5325 (BeerFinder + Faceplant — intermittent, Faceplant crashes 50%)
- [x] Detect that two projects share port 5323 (MySpice + SubSnack — PHP vs Node)
- [x] Detect that two projects share port 5331 (Napper + CPUStorm — Ruby vs Node)
